export interface ListaDeseos {
  ...
  export const ListaDeseosModel = {
    ...
    findByUsuario: async (usuarioId: number): Promise<ListaDeseosConProducto[]> => {
      try {
        const [rows] = await pool.query(`
          ...
          WHERE l.fkCuentaUser = ? AND p.estatus = 'ACTIVO'
        `, [usuarioId]);
    ...
    existsInWishlist: async (usuarioId: number, productoId: number): Promise<boolean> => {
      try {
        const [rows] = await pool.query(`
          ...
          WHERE fkCuentaUser = ? AND fkProducto = ?
    ...
    removeFromWishlist: async (id: number): Promise<boolean> => {
      try {
        const [result] = await pool.query('DELETE FROM listasdedeseos WHERE idLista = ?', [id]);
    ...
    removeProductFromWishlist: async (usuarioId: number, productoId: number): Promise<boolean> => {
      try {
        const [result] = await pool.query('DELETE FROM listasdedeseos WHERE fkCuentaUser = ? AND fkProducto = ?', [usuarioId, productoId]);
        const { affectedRows } = result as { affectedRows: number };
        return affectedRows > 0;
      } catch (error) {
    ...
    clearWishlist: async (usuarioId: number): Promise<boolean> => {
      try {
        const [result] = await pool.query('DELETE FROM listasdedeseos WHERE fkCuentaUser = ?', [usuarioId]);
        const { affectedRows } = result as { affectedRows: number };
        return affectedRows > 0;
      } catch (error) {
